...
export default function VisualizarPedidosCozinha({ navigation }) {
    ...
    useEffect(() => {
        // Escuta em tempo real a coleção 'mesas', filtrando apenas pedidos pendentes
        const unsubscribe = onSnapshot(collection(database, 'mesas'), (querySnapshot) => {
            const lista = [];
            querySnapshot.forEach((docSnap) => {
                const dados = docSnap.data();
                // Exibe apenas pedidos que ainda não foram concluídos pela cozinha
    ...
    const ConcluirPedido = async (pedido) => {
        try {
            // Marca o pedido como concluído pela cozinha e avisa o garçom
            await updateDoc(doc(database, 'mesas', pedido.id), {
    ...
    return (
        <ScrollView style={styles.scrollContainer} contentContainerStyle={styles.scrollContent}>
            ...
            {/* Botão voltar para LoginCozinha */}
            <TouchableOpacity
            ...
            {pedidos.map((pedido) => (
                <View key={pedido.id} style={styles.card}>
                    ...
                    <View style={styles.cardRow}>
                        ...
                        {/* Coluna direita: garçom + botão concluir */}
